  ...
  '(lambda (world)
     (cond ((cons1? $clock1)
            (if (and (zero? $tally)
                     (>= $location min-lower-loc)
                     (not (= $location y2)))
                (let-world (($set-clock1 cdr1))
                  (if (cons1? $clock1)
                      ($goto check-the-lamp)
                      ($goto warn-close)))
                ($goto check-the-lamp)))
           ((cons1? $clock2)
            (let-world (($set-clock2 cdr1))
              (if (cons1? $clock2)
                  ($goto check-the-lamp)
                  ($goto close-the-cave))))
           (else
...
(define-proc 'check-the-lamp
  '(lambda (world)
     (let ((old-limit $limit))
       (let-world (($set-limit (if (= ($prop-of LAMP) c1) cdr1 I)))
         (cond ((and (not (cons1? (c30 cdr1 $limit)))
                     ($here? BATTERIES)
    ...
    (if (yes (nth (car $hint-count) hint-prompt)
        ...
        ((string " but it will cost you ")
         ...
         (if (yes (string "Do you want the hint?")
                  ...
                  ok)
             (let-world ((if (cons1? (c30 cdr1 $limit))
