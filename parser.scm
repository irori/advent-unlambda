...
(define (define-word type meaning . rest)
  (let ((m (if (number? meaning) meaning (lookup-enum meaning)))
    ...
    (for-each
     (lambda (w)
       (trie-put! parser-trie w 0 (list type m aux)))
     words)))
...
(defmacro (motion-word meaning)
  (cons I (lambda (f) (f (lambda (x _ _ _) x) meaning I))))
(defmacro (object-word meaning)
  (cons I (lambda (f) (f (lambda (_ x _ _) x) meaning I))))
(defmacro (action-word meaning)
  (cons I (lambda (f) (f (lambda (_ _ x _) x) meaning I))))
(defmacro (message-word meaning)
  (cons I (lambda (f) (f (lambda (_ _ _ x) x) meaning I))))
